...
BEGIN
	...
	order by count(*)/2 DESC) > 48
		-- tirar error xq supera las 48 hrs
		INSERT INTO Horarios select * from inserted --esta linea esta para q compile. borrarla de aca
	ELSE
		BEGIN
		...
		where H.hora_profesional = @prof_id) > 0
			-- tirar error xq el horario es incompatible con los existentes
			INSERT INTO Horarios select * from inserted --esta linea esta para q compile. borrarla de aca
		ELSE
